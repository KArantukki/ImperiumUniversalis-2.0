...
VertexShader = 
{
	...
	[[
		VS_OUTPUT main( const VS_INPUT v )
		{
			...
			Out.vUV.x += vTimeDirectionSeasonLerp.x * 0.3f * vTimeDirectionSeasonLerp.y;
			Out.vUV.y += vTimeDirectionSeasonLerp.x * 0.2f;
			Out.vUV.x *= 0.05f;
			Out.vSecondaryUV.yx = v.vUV_Tangent.xy;
			Out.vSecondaryUV.x += vTimeDirectionSeasonLerp.x * 0.3f * vTimeDirectionSeasonLerp.y;
...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT In ) : PDX_COLOR
		{
			...
			float3 vColor = lerp( vBottom, vWaterSurface.xyz, vWaterSurface.a * 0.5f );
