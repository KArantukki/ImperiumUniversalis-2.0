...
const static float SNOW_START_HEIGHT 	= 18.6f;
const static float SNOW_RIDGE_START_HEIGHT 	= 17.0f;
const static float SNOW_NORMAL_START 	= 0.89f;
const static float3 SNOW_COLOR = float3( 0.8f, 0.8f, 0.8f );
const static float3 SNOW_WATER_COLOR = float3( 0.7f, 0.7f, 0.7f );
...
float3 ApplySnow( float3 vColor, float3 vPos, inout float3 vNormal, float4 vFoWColor, in sampler2D FoWDiffuse, float3 vSnowColor )
{
	...
	float vSnow = saturate( saturate( vNoise - ( 0.1f - vIsSnow ) ) * 48.0f );
